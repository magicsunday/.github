...
# would not be caught by the sibling suite at all) is untested by every
# case above: each drives the function via `$(...)` capture in a script
# that never sets `-e`, which cannot exercise it - command substitution
# does not propagate errexit into itself, so an unguarded `rm` failure
# there wouldn't abort anything even without `|| true`. The real
# production caller (code-scanning.yml) invokes the function as a bare
# statement under `set -euo pipefail`. Mutation-confirmed: removing the
